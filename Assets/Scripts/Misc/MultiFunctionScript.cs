...
public class MultiFunctionScript : MonoBehaviour
{
    ...
    public void ChangeColorTo(uint index)
    {
        if (index > colorTable.Count) return;
        GetComponent<Renderer>().material = colorTable[(int)index];
    }
    ...
    public void LoadSceneFromList(uint index)
    {
        if (index > sceneNames.Count) return;
        SceneManager.LoadScene(sceneNames[(int)index]);
    }
}
